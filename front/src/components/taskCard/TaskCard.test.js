...
describe('TaskCard - Validações e Testes de UI', () => {
  ...
  describe('Validação de Renderização Básica', () => {
    ...
    test('deve mostrar setor correto', () => {
      ...
      expect(screen.getByText('TI')).toBeInTheDocument()
      expect(screen.getByText(/Setor: TI/i)).toBeInTheDocument()
    })
    ...
    test('deve mostrar cores corretas para diferentes prioridades', () => {
      ...
      tasks.forEach((task, index) => {
        const { rerender } = render(<TaskCard task={task} color="border-red-400" users={mockUsers} refreshTasks={mockRefreshTasks} />)
        
        const priorityIndicator = screen.getByRole('status', { 
          name: new RegExp(`${priorities[index]} prioridade`, 'i') 
        })
  ...
  describe('Validação de Integração com Backend', () => {
    ...
    test('deve mostrar erro quando a API falha', async () => {
      axios.patch.mockRejectedValueOnce({ response: { data: { message: 'Erro ao atualizar tarefa' } } })
